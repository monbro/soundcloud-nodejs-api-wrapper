...
Client.prototype.sendJson = function(bool) {
	if(bool) {
		this.settings.sendJson = true;
	} else {
		this.settings.sendJson = false;
	}
}
...
Client.prototype.request = function(method, path, post_body, filters, callback) {
	...
	// add the authentication header, so we can actually modify account content,
	// also check if we are sending json or form fields
	if(this.settings.sendJson) {
		var header = {
	...
	this.oauth._request(method, urlString, header, post_body, this.settings.access_token, oauthCallback);
}
